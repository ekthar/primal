...
export default function AdminUsers() {
  ...
  if (user?.role !== "admin") {
    return <div className="p-10 text-sm text-secondary-muted">Only admin can manage users.</div>;
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
      <div>
        ...
        <h1 className="font-display text-3xl sm:text-4xl font-semibold tracking-tight mt-1">User and reviewer management</h1>
        <p className="text-sm text-secondary-muted mt-2">Create reviewer, club, applicant, or admin accounts from one place.</p>
      </div>
      ...
      <section className="rounded-3xl border border-border bg-surface elev-card p-6">
        ...
        <form className="mt-4 grid md:grid-cols-2 gap-4" onSubmit={handleCreate}>
          <div>
            <Label className="text-[11px] uppercase tracking-wider font-semibold text-secondary-muted">Name</Label>
            <Input
          ...
          <div>
            <Label className="text-[11px] uppercase tracking-wider font-semibold text-secondary-muted">Role</Label>
      ...
      <section className="rounded-3xl border border-border bg-surface elev-card p-6">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <h2 className="font-display text-2xl font-semibold tracking-tight">Accounts</h2>
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" className="h-10" onClick={loadUsers}>Refresh</Button>
          </div>
        </div>

        <div className="mt-4 overflow-x-auto">
          {loading ? (
            <div className="py-6 text-sm text-secondary-muted">Loading users...</div>
          ) : (
          ...
          {!loading && filteredUsers.length === 0 && (
            <div className="py-6 text-sm text-secondary-muted">No users found.</div>
          )}
        </div>
      </section>
    </div>
